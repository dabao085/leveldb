...
std::string test_block_builder() {
  ...
  // Write block_contents to binary file
  std::ofstream outfile("block_data.bin", std::ios::binary);
  outfile.write(block_contents.data(), block_contents.size());
  outfile.close();

  std::cout << "block_contents:" << block_contents << std::endl;
...
void test_block(const std::string& block_contents) {
  ...
  while (block_iter->Valid()) {
    ...
    std::cout << block_iter->key().ToString() << " -> "
              << block_iter->value().ToString() << std::endl;
    block_iter->Next();
  }
...
int main() {
  const std::string block_contents = test_block_builder();
